...
const App = () => {
  ...
  const handleSubmit = () => {
    ...
    const aadharNumber = transcribedText["Aadhaar Number"];
    if (unfilledFields.length === 0) {
      if (aadharNumber.length !== 12) {
        ...
        axios
          .post("https://mockapi-hhv3.onrender.com/onefinitylabsApi", formData)
          .then((response) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-500 to-yellow-500 flex flex-col justify-center items-center">
      <div className="border border-gray-300 rounded-l-md shadow-md p-6">
          ...
          {fields.map((field, index) => (
            <div key={index} className="flex items-center">
              <div className="flex flex-col">
                ...
                <input
                  type={field === "Aadhaar Number" ? "number" : "text"}
                  placeholder={field}
                  ...
                  className="border border-gray-300 rounded-l-md px-4 py-2 w-54 shadow-md"
                  required
                />
              </div>
              <button
        ...
        <button
          ...
          className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4 shadow-md w-full"
        >
          Submit
        </button>
